...
func opExtCodeCopyEIP4762(pc *uint64, evm *EVM, scope *ScopeContext) ([]byte, error) {
	...
	consumed, wanted := evm.AccessEvents.CodeChunksRangeGas(addr, evm.StateDB.GetCodeHash(addr), copyOffset, nonPaddedCopyLength, uint64(len(code)), false, scope.Contract.Gas)
	...
	*pc += 1
	if *pc < codeLen {
		...
		if !scope.Contract.IsDeployment && !scope.Contract.IsSystemCall && *pc%31 == 0 {
			...
			consumed, wanted := evm.AccessEvents.CodeChunksRangeGas(contractAddr, scope.Contract.CodeHash, *pc+1, uint64(1), uint64(len(scope.Contract.Code)), false, scope.Contract.Gas)
...
func makePushEIP4762(size uint64, pushByteSize int) executionFunc {
	return func(pc *uint64, evm *EVM, scope *ScopeContext) ([]byte, error) {
		...
		if !scope.Contract.IsDeployment && !scope.Contract.IsSystemCall {
			contractAddr := scope.Contract.Address()
			consumed, wanted := evm.AccessEvents.CodeChunksRangeGas(contractAddr, scope.Contract.CodeHash, uint64(start), uint64(pushByteSize), uint64(len(scope.Contract.Code)), false, scope.Contract.Gas)
			scope.Contract.UseGas(consumed, evm.Config.Tracer, tracing.GasChangeUnspecified)
			if consumed < wanted {
				return nil, ErrOutOfGas
			}
