...
// reencode round-trips a proof through its wire form.
//
// Verifying a proof straight out of the prover is weaker than it reads: the
// canonical-bitmap rule lives in the decoder, so a test that skips it never
// checks that the prover emits the one form its own verifier accepts.
...
// recordsDB writes the records of tr into a fresh witness-backed database and
// returns a trie opened on the same root, which is what a stateless verifier
// does with a proof it has checked.
...
// TestWriteRecordsRoundTrip is the gate on the serializer: a proof, verified,
// written out as database records and read back has to answer exactly what it
// covered and fault on everything else.
//
// It is the only test that runs the writer's path arithmetic against the
// reader's. They are separate walks - the writer accumulates positions as it
// descends, the reader slices them out of the key it is looking for - and
// nothing downstream would catch a disagreement: a binary node's hash is not the
// hash of its own bytes, so pathdb adopts whatever hash the parent claimed and a
// record one level off surfaces only as a root that cannot match.
...
func TestWriteRecordsSkipsStubs(t *testing.T) {
	...
	// Both roots verified, so both cover the same root - but the whole-stem proof
	// holds the group as one record where the single-chunk proof holds the
	// expansion around one leaf. If the two ever produce the same records, the
	// stem request stopped meaning anything.
