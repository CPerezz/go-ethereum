...
func TestPartialStemRefusesGroupOperations(t *testing.T) {
	for _, tc := range partialCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Dropping the stem wholesale takes delStem rather than insStem,
			// which the delete case above exercises. This is the route
			// DeleteAccount takes.
...
// TestProofRequestKinds pins what each request kind covers. A proof over keys
// alone can only answer reads, which is not enough for a verifier that has to
// re-execute a block: writing to a stem needs the whole group, and a structural
// change needs a node no key names.
