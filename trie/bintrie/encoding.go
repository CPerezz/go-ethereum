...
func serializeNodeErr(n binaryNode, pos int) ([]byte, error) {
	...
	case *groupNode:
		...
		//
		// Bounded by the stem rather than by the deepest legal path, matching
		// what decodeNode accepts: a looser bound here would write a record that
		// only failed on the way back in, reported as a length error.
		if pos > 8*len(n.stem) {
