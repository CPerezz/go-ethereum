...
func (db *Database) Enable(root common.Hash) error {
	...
	// Enable resets the database to a state some other writer has healed into
	// place, which for the binary tree would leave flat state describing the
	// state before the sync. It cannot be regenerated from the tree, so there
	// would be no way back. A binary-tree sync has to deliver flat state
	// alongside the nodes; until one exists, refuse.
	if db.isPBT {
	...
	// No binary-tree term here: this function refuses that scheme outright above.
