...
func (db *Database) Update(root common.Hash, parentRoot common.Hash, block uint64, nodes *trienode.MergedNodeSet, states *StateSetWithOrigin) error {
	...
	// The binary tree relies on this refusal: a witness-backed trie skips the
	// deletion bookkeeping only a commit reads.
	if db.config.WitnessOnly {
		return errDatabaseWitnessOnly
	}
...
// witness. Such a database holds only what one block resolved and is never
// committed to, which lets the trie skip bookkeeping only a commit reads.
func (db *Database) WitnessOnly() bool { return db.config.WitnessOnly }
